...
async function sendDailyBackupSuccessEmail({
  ...
  const collectionRows = Object.entries(metadata.collectionStats || {}).map(([colName, count]) => {
    const arabicNames = {
      ...
      leave_requests: "\u0637\u0644\u0628\u0627\u062A \u0627\u0644\u0625\u062C\u0627\u0632\u0627\u062A (Leave Requests)",
...
function validateLeaveSettlement(data) {
  ...
  if (requestedDays > totalAvailable + 1e-3) {
    throw new Error(
      `\u062A\u062C\u0627\u0648\u0632 \u0627\u0644\u0631\u0635\u064A\u062F: \u0627\u0644\u0631\u0635\u064A\u062F \u0627\u0644\u0645\u062A\u0627\u062D (${totalAvailable.toFixed(2)}) \u0644\u0627 \u064A\u0643\u0641\u064A \u0644\u0637\u0644\u0628 (${requestedDays.toFixed(2)}) \u064A\u0648\u0645.`
    );
  }
...
// src/config/kuwaitLaborConstants.ts
var DAILY_WAGE_DIVISOR = 26;
var KWD_DECIMALS = 3;
var ACCRUAL_2026_MONTHLY_RATE = 2.5;
var ACCRUAL_2026_ANNUAL_CAP = 30;
var ACCRUAL_2026_EPOCH = "2026-01-01";

// src/utils/leaveAccrual2026.ts
function cleanAccrualDays(days) {
  if (days === void 0 || days === null || isNaN(days)) return 0;
  return Number((Math.round((days + Number.EPSILON) * 100) / 100).toFixed(2));
}
function resolveJoinDate(joinDateStr) {
...
function resolveAsOfDate(asOfDate) {
  if (!asOfDate) return /* @__PURE__ */ new Date();
  const asOf = asOfDate instanceof Date ? asOfDate : new Date(asOfDate);
  return isNaN(asOf.getTime()) ? /* @__PURE__ */ new Date() : asOf;
}
function computeAccrual2026Unified(joinDateStr, asOfDate) {
...
function extractEmployeeJoinDate(employee) {
  if (!employee) return void 0;
  if (typeof employee === "string") return employee;
  if (employee instanceof Date) return employee.toISOString().split("T")[0];
  return employee.joinDate || employee.hireDate || employee.date_start || employee.startDate || employee.openingLeaveDate || employee.openingDate;
}

// src/utils/leaveModel.ts
var LEAVE_TYPE_MAP = {
  ANNUAL: "ANNUAL",
  ANNUA: "ANNUAL",
  SICK: "SICK",
...
// src/utils/kuwaitPayrollMath.ts
function cleanKwdAmount(amount) {
  if (amount === void 0 || amount === null || isNaN(amount)) return 0;
  return Number((Math.round((amount + Number.EPSILON) * 1e3) / 1e3).toFixed(KWD_DECIMALS));
}
function calculateKuwaitDailyRate(basicWage) {
  if (!basicWage || basicWage <= 0) return 0;
  return cleanKwdAmount(basicWage / DAILY_WAGE_DIVISOR);
}

// server/leaveCalculatorServer.ts
function getApprovedLeaveDays(leave) {
  return Number(
    leave.paidDays || leave.days || leave.durationDays || leave.numberOfDays || leave.totalDays || 0
  );
}
function deductFromBuckets(buckets, daysToDeduct, bucketFilter) {
  let remaining = daysToDeduct;
  let deducted = 0;
  for (const bucket of buckets.filter(bucketFilter)) {
    if (remaining <= 0) break;
    const canTake = Math.min(bucket.remainingDays, remaining);
    if (canTake <= 0) continue;
    bucket.consumedDays = cleanDays(bucket.consumedDays + canTake);
    bucket.remainingDays = cleanDays(bucket.remainingDays - canTake);
    deducted = cleanDays(deducted + canTake);
    remaining = cleanDays(remaining - canTake);
  }
  return deducted;
}
...
function calculateServerAccrued2026(emp, asOfDateStr) {
  ...
  return cleanDays(computeAccrual2026Unified(joinDate, asOfDateStr || /* @__PURE__ */ new Date()));
}
function calculateServerOpeningBalance(emp) {
  if (!emp) return 0;
  const explicitVal = emp.carriedOverLeave2025 ?? emp.carriedOverBalance ?? emp.carriedOverLeaveBalance ?? emp.aysed_carried_over ?? emp.openingLeaveBalance ?? emp.openingBalance ?? emp.initialLeaveBalance ?? emp.carriedOver;
...
function calculateServerFifoBalance(employee, allocations = [], leaves = [], contract, asOfDateStr) {
  ...
  const approvedLeaves = leaves.filter(
    (l) => !l.isHistorical && (l.employeeId === empId || l.employeeId === empCode) && (l.status === "APPROVED" || l.state === "validate" || l.state === "approved")
  );
  let totalConsumed = 0;
  for (const leave of approvedLeaves) {
    const leaveType = normalizeLeaveType(leave.leaveType || "ANNUAL");
    if (leaveType === "UNPAID" || leaveType === "SICK" || leaveType === "BEREAVEMENT") {
      continue;
    }
    const daysToDeduct = getApprovedLeaveDays(leave);
    if (daysToDeduct <= 0) continue;
    if (leaveType === "COMPENSATORY") {
      totalConsumed += deductFromBuckets(
        buckets,
        daysToDeduct,
        (bucket) => bucket.type === "compensatory"
      );
      continue;
    }
    totalConsumed += deductFromBuckets(
  ...
  const dailyWageRate = basicSalary > 0 ? calculateKuwaitDailyRate(basicSalary) : 0;
  const cashSettlementAmount = cleanKwdAmount(netAvailable * dailyWageRate);
  return {
    employeeId: empId,
...
function calculateServerSettlement(params) {
  ...
  const totalBalanceBefore = balance.totalAvailableDays;
  const paidLeaveDays = Math.min(totalBalanceBefore, consumedDays);
  const unpaidLeaveDays = Math.max(0, consumedDays - totalBalanceBefore);
  const balanceAfter = cleanDays(Math.max(0, totalBalanceBefore - paidLeaveDays));
  const proratedSalaryAmount = includeProratedSalary && workedDaysInMonth > 0 ? cleanKwdAmount(workedDaysInMonth * dailyWage) : 0;
...
function getSupabaseAdmin() {
  ...
  supabaseAdminClient = (0, import_supabase_js.createClient)(cleanUrl, serviceKey.trim(), {
    auth: {
...
app.post("/api/biometrics/sync", import_express.default.json(), (req, res) => {
  try {
...
app.get("/api/system/env-health", async (req, res) => {
  try {
    ...
    const hasServiceAccount = Boolean(
      process.env.FIREBASE_SERVICE_ACCOUNT && process.env.FIREBASE_SERVICE_ACCOUNT.trim().length > 0 || process.env.FIREBASE_CLIENT_EMAIL && process.env.FIREBASE_PRIVATE_KEY
    );
...
function normalizeOcrDataServer(parsed) {
    ...
    for (const line of lines) {
      ...
      if (parts.length >= 2) {
        ...
        if (empCode && punchTime) {
          const dateStr = punchTime.split(" ")[0] || (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
...
app.post("/api/send-email", import_express.default.json(), async (req, res) => {
  ...
  try {
    const transporter2 = import_nodemailer2.default.createTransport({
      ...
      // 4 seconds timeout to fail faster than proxy
      greetingTimeout: 4e3,
      socketTimeout: 4e3
    });
...
app.post("/api/generate-pam-contract", import_express.default.json(), async (req, res) => {
  try {
    let shapeAr = function(text) {
      if (!text) return "";
      const hasArabic = /[\u0600-\u06FF]/.test(text);
      if (!hasArabic) return text;
      let shaperObj = reshaper.ArabicShaper || reshaper;
      let reshaped = text;
    ...
    const templatePath = import_path.default.join(process.cwd(), "public", "pam_contract_form_2.pdf");
    const fontPath = import_path.default.join(process.cwd(), "public", "fonts", "Amiri-Bold.ttf");
    ...
    const draw = (p, txt, x, y, sz = 8.5, right = true) => {
      if (!txt) return;
...
app.post("/api/auth/send-2fa-otp", import_express.default.json(), async (req, res) => {
  try {
...
app.post("/api/auth/verify-2fa-otp", import_express.default.json(), (req, res) => {
  try {
    const { userId, otpCode } = req.body;
    const now = /* @__PURE__ */ new Date();
    const record = userOtpCodesStore.find(
      (r) => (String(r.user_id) === String(userId) || !userId) && r.otp_code === String(otpCode).trim() && !r.is_used && r.expires_at > now
    );
    if (!record) {
...
INSERT INTO system_settings (
  ...
  res.setHeader("Content-Disposition", `attachment; filename="aysed_settings_backup_${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.json"`);
...
async function startServer() {
  ...
  function runStartupEnvironmentAudit() {
    console.log("=================================================");
    console.log("\u{1F680} [AYSED HR 2026] Running Environment Validation Audit");
    console.log("=================================================");

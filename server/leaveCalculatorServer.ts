...
import { computeAccrual2026Unified, extractEmployeeJoinDate } from '../src/utils/leaveAccrual2026';
import { normalizeLeaveType } from '../src/utils/leaveModel';
import { calculateKuwaitDailyRate, cleanKwdAmount as cleanKwdUnified } from '../src/utils/kuwaitPayrollMath';
...
function getApprovedLeaveDays(leave: LeaveRequest): number {
  return Number(
    (leave as any).paidDays ||
      (leave as any).days ||
      (leave as any).durationDays ||
...
function deductFromBuckets(
  buckets: FifoBucket[],
  daysToDeduct: number,
  bucketFilter: (bucket: FifoBucket) => boolean
): number {
...
export function calculateServerFifoBalance(
  ...
  // فلترة الإجازات المعتمدة — فقط ANNUAL و COMPENSATORY تستهلك الرصيد
  ...
  const cashSettlementAmount = cleanKwdUnified(netAvailable * dailyWageRate);
...
export function calculateServerSettlement(params: ServerSettlementCalculationParams): ServerSettlementCalculationResult {
  ...
  const proratedSalaryAmount = includeProratedSalary && workedDaysInMonth > 0 
    ? cleanKwdUnified(workedDaysInMonth * dailyWage)
    : 0;

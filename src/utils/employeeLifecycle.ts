...
export function normalizeEmployeeStatus(rawStatus?: string | null): EmployeeLifecycleStatus {
  ...
  // Active (English + Arabic labels used across the app)
  if (
    ['active', 'act', 'على رأس العمل', 'على راس العمل', 'ساري', 'مباشر', 'نشط'].includes(s) ||
    s.includes('رأس العمل') ||
    s.includes('راس العمل')
  ) {
    return 'ACTIVE';
  }

...
import { fetchLeavesFromSupabase, upsertLeaveToSupabase } from './leaveSupabaseSync';
export { toLeaveDbRow, fromLeaveDbRow, type LeaveDbRecord } from './leaveDbAdapter';
...
export const TenantDatabaseService = {
  ...
  /**
   * Delete an Employee from all persistent stores and purge all related records (contracts, leave_requests, commencements)
   */
  async deleteEmployee(employeeId: string, companyId?: string): Promise<boolean> {
    ...
    try {
      ...
      console.warn('[TenantDatabaseService] Firestore delete employee failed:', fsErr);
      return false;
    }
  ...
  async saveLeave(leave: LeaveRequest, targetCompanyId?: string): Promise<boolean> {
    ...
    try {
      ...
      await setDoc(doc(db, 'leave_requests', leave.id), cleanDoc, { merge: true });
      await upsertLeaveToSupabase({ ...leave, companyId: compId }, compId).catch(() => false);
      return true;
    } catch (fsErr) {
  ...
  async getLeavesByTenant(companyId: string): Promise<LeaveRequest[]> {
    ...
    try {
      ...
      const snap = await getDocs(q);
      const firestoreLeaves = snap.docs.map(d => ({ ...d.data(), id: d.id } as LeaveRequest));
      if (firestoreLeaves.length > 0) {
        return firestoreLeaves;
      }

      return await fetchLeavesFromSupabase(companyId);
    } catch (fsErr) {
      console.warn('[TenantDatabaseService] Firestore leaves query error:', fsErr);
      return fetchLeavesFromSupabase(companyId);
    }
  },
    ...
    try {
      ...
      if (typeof window !== 'undefined' && window.localStorage) {
        ...
        for (const req of odooRequests) {
          await setDoc(doc(db, 'leave_requests', req.id), cleanFirestoreData({ ...req, companyId }), { merge: true });
        }

...
import { upsertLeaveAllocationToSupabase, upsertLeaveToSupabase } from './leaveSupabaseSync';
...
export async function approveLeaveRequest(
  ...
  return runTransaction(db, async transaction => {
    ...
    if (requestSnap.exists()) {
      void upsertLeaveToSupabase({ id: requestSnap.id, ...requestSnap.data() } as any, companyId).catch(() => false);
    }

    const allocationIds = new Set(result.allocationBreakdown.map(item => item.allocationId));
    await Promise.all(
      [...allocationIds].map(async allocationId => {
        const allocationSnap = await getDoc(doc(db, 'leave_allocations', allocationId));
        if (!allocationSnap.exists()) return;
        void upsertLeaveAllocationToSupabase(
          { id: allocationSnap.id, ...allocationSnap.data() },
          companyId
        ).catch(() => false);
      })
    );

        ...
        <table style="width: 100%; border-collapse: collapse; text-align: center; font-size: 11px;">
          <tr>
            <td style="background-color: #fff7ed; border: 1px solid #fed7aa; padding: 12px; border-radius: 6px; width: 16.66%;">
              ...
              <div style="font-size: 15px; font-weight: bold; color: #c2410c;">-${fifoBreakdown?.consumedFromCarried ?? 0} يوم</div>
            </td>
            <td style="background-color: #faf5ff; border: 1px solid #e9d5ff; padding: 12px; border-radius: 6px; width: 16.66%;">
              ...
              <div style="font-size: 15px; font-weight: bold; color: #7e22ce;">-${fifoBreakdown?.consumedFromAccrued ?? 0} يوم</div>
            </td>
            <td style="background-color: #ecfdf5; border: 1px solid #a7f3d0; padding: 12px; border-radius: 6px; width: 16.66%;">
              ...
              <div style="font-size: 15px; font-weight: bold; color: #047857;">-${fifoBreakdown?.consumedFromComp ?? 0} يوم</div>
            </td>
            <td style="background-color: #f8fafc; border: 1px solid #cbd5e1; padding: 12px; border-radius: 6px; width: 16.66%;">
              ...
              <div style="font-size: 15px; font-weight: bold; color: #334155;">${fifoBreakdown?.remainingCarried ?? 0} يوم</div>
            </td>
            <td style="background-color: #eef2ff; border: 1px solid #c7d2fe; padding: 12px; border-radius: 6px; width: 16.66%;">
              ...
              <div style="font-size: 15px; font-weight: bold; color: #4338ca;">${fifoBreakdown?.remainingAccrued ?? 0} يوم</div>
            </td>
            <td style="background-color: #f0fdfa; border: 1px solid #99f6e4; padding: 12px; border-radius: 6px; width: 16.66%;">
              ...
              <div style="font-size: 15px; font-weight: bold; color: #0f766e;">${fifoBreakdown?.remainingComp ?? 0} يوم</div>
            </td>
          </tr>
        </table>
      </div>
...
export function EmployeesApp(props?: any) {
  ...
  const requestDeleteEmployee = (id: string, name: string, e?: React.MouseEvent) => {
    if (e) e.stopPropagation();
    setEmployeePendingDelete({ id, name: name || id });
  };

  const confirmDeleteEmployee = async () => {
    if (!employeePendingDelete) return;
    const { id, name } = employeePendingDelete;
    setIsDeletingEmployee(true);
    try {
      const deleted = await TenantDatabaseService.deleteEmployee(id, currentCompanyId);
      if (!deleted) {
        toast.error(`تعذر حذف الموظف (${name}) من قاعدة البيانات`);
        return;
      }

      setEmployees(prev => prev.filter(emp => emp.id !== id));
      setContracts(prev =>
        prev.filter((c: any) => c.employeeId !== id && c.id !== `contract-${id}`)
      );
      setCommencements(prev =>
        prev.filter((c: any) => c.employeeId !== id && c.id !== `commencement-${id}`)
      );
      ...
      toast.error(`حدث خطأ أثناء حذف الموظف (${name})`);
    } finally {
  ...
  const handleSaveEmployee = async (updatedEmp: any) => {
    ...
    setEmployees(prev => {
      const exists = prev.some(e => e.id === viewEmployee.id);
      return exists
        ? prev.map(e => (e.id === viewEmployee.id ? viewEmployee : e))
        : [viewEmployee, ...prev];
    });
    ...
    toast.success(`تم حفظ بيانات الموظف (${viewEmployee.nameAr}) وتحديث الراتب (${totalSal.toFixed(3)} د.ك) بنجاح`);
  };
    ...
    >
      <button
        ...
        onClick={() => {
          setShowActionsDropdown(false);
          handleTriggerPrint(
            activeTab === 'directory' ? 'سجل الموظفين الشامل' : 'تقرير المنشأة',
            { nameAr: activeCompany?.nameAr || activeCompany?.name || 'تقرير المنشأة' }
          );
        }}
  ...
  return (
    <div className="flex-1 flex flex-col w-full font-sans select-none text-slate-800 min-h-0" dir="rtl">
      ...
      {selectedEmployee && activeTab === 'directory' ? (
        <OdooEmployeeDetailView
          employee={selectedEmployee}
          activeCompany={activeCompany}
          onSave={handleSaveEmployee}
          onBack={() => setSelectedEmployee(null)}
          onDelete={(id, name) => requestDeleteEmployee(id, name)}
          ...
          commencementRecord={
            commencements.find((c: any) => String(c.employeeId) === String(selectedEmployee?.id)) || null
          }
        />
      ) : (
        <EmployeesAppChrome
          ...
          companyLabel={activeCompany?.nameAr || activeCompany?.name || 'المنشأة'}
          showDirectoryTools={activeTab === 'directory'}
          showViewToggle={activeTab === 'directory'}
          viewMode={viewMode}
          ...
          onCreateEmployee={handleCreateNewEmployee}
          actionsMenuOpen={showActionsDropdown}
                            ...
                            {activeCardMenuId === emp.id && (
                              ...
                              >
                                ...
                                <button
                                  onClick={(e) => {
                                    setActiveCardMenuId(null);
                                    requestDeleteEmployee(emp.id, emp.nameAr, e);
                                  }}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-3xs">
                ...
                <div className="text-2xl font-black text-slate-800 font-mono">
                  {visibleEmployees.filter(e => normalizeEmployeeStatus(e.status) === 'ACTIVE').length}
                </div>
              ...
              <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-3xs">
                ...
                <div className="text-2xl font-black text-slate-800 font-mono">
                  {visibleEmployees.filter(e => normalizeEmployeeStatus(e.status) !== 'ACTIVE' && !e.isDeleted).length}
                </div>
              ...
              <div className="mt-8 pt-6 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="flex items-start gap-2.5 text-xs text-slate-500 max-w-xl">
                  ...
                  <p className="leading-relaxed text-[11px]">
                    <strong>تنظيم حوكمة الموارد البشرية:</strong> إلغاء إدخال بيانات المباشرة اليدوية في دليل الموظفين يمنع بنسبة 100% حدوث تباين أو تضارب في تواريخ المباشرة الفعلية التي تبنى عليها عقود العمل وامتثال قانون العمل بدولة الكويت.
                  </p>
                </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4" dir="rtl">
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl border border-slate-200 overflow-hidden">
            <div className="p-5 bg-rose-50 border-b border-rose-100 flex items-center gap-3">
              ...
              <div>
                <h3 className="font-bold text-rose-950 text-sm">تأكيد حذف ملف الموظف</h3>
                <p className="text-[11px] text-rose-700">إجراء نهائي — سيُحذف الموظف وسجلاته المرتبطة</p>
              </div>
            </div>
            <div className="p-6 space-y-4 text-xs text-slate-700">
              <p>
                هل أنت متأكد من حذف الموظف{' '}
                <strong className="text-slate-900">{employeePendingDelete.name}</strong> نهائياً؟
              </p>
              <div className="flex items-center gap-3">
                <button
                  type="button"
                  ...
                  className="flex-1 bg-rose-600 hover:bg-rose-700 text-white py-3 px-4 rounded-xl font-bold text-xs transition disabled:opacity-50"
                >
                  {isDeletingEmployee ? 'جارِ الحذف...' : 'نعم، احذف نهائياً'}
                </button>
                <button
                  type="button"
                  onClick={() => setEmployeePendingDelete(null)}
                  disabled={isDeletingEmployee}
                  className="px-5 py-3 border border-slate-300 hover:bg-slate-50 text-slate-700 rounded-xl font-bold text-xs transition"
                >
                  إلغاء
                </button>
              </div>

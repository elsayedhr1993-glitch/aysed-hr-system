...
export const OdooSettingsFull: React.FC = () => {
  ...
  const [isLeaveWizardOpen, setIsLeaveWizardOpen] = useState(false);
  const companyId = activeCompany?.id || '';
  const leavePolicyCacheKey = companyId ? `${TIMEOFF_POLICY_STORAGE_KEY}_${companyId}` : TIMEOFF_POLICY_STORAGE_KEY;

  const reloadLeavePolicy = React.useCallback(() => {
    if (!companyId) {
      setLeavePolicy(getLeaveMasterPolicy());
      return;
    }
    void loadTenantPolicy(companyId, 'leave_policy', getLeaveMasterPolicy, leavePolicyCacheKey).then(setLeavePolicy);
  }, [companyId, leavePolicyCacheKey]);
  ...
  React.useEffect(() => {
    const onPolicyUpdate = () => reloadLeavePolicy();
    window.addEventListener('timeoff_policy_updated', onPolicyUpdate);
    return () => window.removeEventListener('timeoff_policy_updated', onPolicyUpdate);
  }, [reloadLeavePolicy]);
  ...
  const handleSave = () => {
    ...
    toast.success('تم حفظ وتحديث بيانات المنشأة في Firestore ومزامنتها في الشريط العلوي بنجاح');
    setSaveSuccess(true);
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-800 font-sans dir-rtl pb-24" dir="rtl">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 flex flex-col md:flex-row gap-6">
        ...
        <div className="flex-1 space-y-6">
          ...
          {/* Section 3: Leave policy (Firestore tenant policy — single source of truth) */}
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-xs p-6 space-y-6">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h2 className="text-sm font-bold text-slate-900">سياسة الإجازات المعتمدة للمنشأة</h2>
                    <p className="text-[11px] text-slate-500">
                      مصدر الحقيقة: معالج السياسة + Firestore (<span className="font-mono">leave_policy</span>) — نفس إعدادات تطبيق الإجازات.
                    </p>
                  </div>
                </div>
                <button
                  type="button"
                  onClick={() => setIsLeaveWizardOpen(true)}
                  className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#714B67] hover:bg-[#583a50] text-white text-xs font-bold cursor-pointer"
                >
                  <Sliders size={14} />
                  فتح معالج سياسة الإجازات
                </button>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                <div className="p-3 bg-teal-50/50 rounded-xl border border-teal-200">
                  <div className="text-[10px] text-slate-500 font-bold">الرصيد السنوي</div>
                  <div className="text-lg font-black text-teal-900 font-mono">{leavePolicy.annualDays.toFixed(1)} يوم</div>
                </div>
                <div className="p-3 bg-teal-50/50 rounded-xl border border-teal-200">
                  <div className="text-[10px] text-slate-500 font-bold">التراكم الشهري (م70)</div>
                  <div className="text-lg font-black text-teal-900 font-mono">+{leavePolicy.monthlyAccrualRate} يوم/شهر</div>
                </div>
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
                  <div className="text-[10px] text-slate-500 font-bold">سقف الترحيل</div>
                  <div className="text-lg font-black text-slate-900 font-mono">{leavePolicy.maxCarryoverDays} يوم</div>
                </div>
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
                  <div className="text-[10px] text-slate-500 font-bold">الحالة</div>
                  <div className="text-sm font-bold text-emerald-700 flex items-center gap-1 mt-1">
                    <CheckCircle2 size={14} />
                    {leavePolicy.isActivated ? 'مفعّلة ومحفوظة سحابياً' : 'غير مفعّلة'}
                  </div>
                  <div className="text-[10px] text-slate-400 mt-1 font-mono truncate" title={leavePolicy.lastUpdated}>
                    آخر تحديث: {leavePolicy.lastUpdated ? new Date(leavePolicy.lastUpdated).toLocaleString('ar-KW') : '—'}
                  </div>
                </div>
              </div>

              <div className="p-4 bg-indigo-50 border border-indigo-200 rounded-xl text-xs text-indigo-900 flex items-start gap-2">
                <Info size={16} className="shrink-0 mt-0.5" />
                <p>
                  التعديلات على التراكم، الإجازات المرضية، الموافقات، وربط الرواتب تتم حصرياً عبر المعالج متعدد الخطوات.
                  لا تُخزَّن حقول الإجازات في <span className="font-mono">company_settings</span> لتجنب التعارض مع محرك الإجازات.
                </p>
              </div>
            </div>
          )}
      ...
      <LeavePolicyWizardModal
        isOpen={isLeaveWizardOpen}
        onClose={() => setIsLeaveWizardOpen(false)}
        onSaved={(policy) => {

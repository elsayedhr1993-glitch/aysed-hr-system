...
import { calculateKuwaitDailyRate, calculateKuwaitLeaveCashAmount, cleanKwdAmount } from '../utils/kuwaitPayrollMath';
...
export const OdooLeaveSettlementApp: React.FC<OdooLeaveSettlementAppProps> = ({ isModal = false, onClose, leaveRequest = null }) => {
  ...
  const calculateAuto = () => {
    const basicSalary = Number((formData as any).basicSalary ?? formData.totalSalary ?? 0);
    const dailyRate = calculateKuwaitDailyRate(basicSalary);
    const workedAmount = cleanKwdAmount(formData.workedDays * dailyRate);
    const leaveAdvance = calculateKuwaitLeaveCashAmount(formData.leaveDays, basicSalary);
    const total = cleanKwdAmount(workedAmount + leaveAdvance + formData.ticket - formData.deductions);
    return {
      workedAmount,
      leaveAdvance,
      net: total,
